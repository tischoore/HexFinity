...
Each piece stays a single rigid body — it isn't forced flat, but its own mesh is never reshaped either: it tilts (pitch/roll, on top of your own scroll-wheel yaw) so it *rests* on the hovered hex's live surface under its authored **Corners** footprint (see **Define Corners** under [Settings](#settings)), the same way a rigid flat object settles onto an uneven floor — touching at whichever corners are locally highest (generically 3 of them) and never sinking below the surface anywhere. A footprint with more than 3 corners over genuinely hilly terrain will generically leave one or more corners floating a small gap above the surface instead of touching it too, since a rigid body only has enough freedom to satisfy 3 height constraints at once (the same reason a 4-legged table wobbles on an uneven floor) — but no corner ever ends up buried below the surface. That's accepted as-is, since a placed segment always stays exactly the prefab it was authored as. Where it snaps to a connector (a previous piece's open end, or a hex edge), the fit is pinned to land exactly on that point, so chained joints never gap. A segment authored before Corners existed, or with fewer than 3, places flat as before. If you need a specific placement's corners to match the surface exactly, that's a separate, manual step afterward — see [Conform to Hex (manual lattice)](#conform-to-hex-manual-lattice) for the Lattice-based approach (today scoped to imported terrain objects).
...
A collapsed **Settings** box sits at the very top of the panel, above the generation menu — visible whether or not a map currently exists. With no segment currently being authored it holds two buttons: **Add Path Segment Type**, an authoring workflow for pre-built path-segment STLs (bridges, junctions, etc.) that draws and records their own footprint corners and connector waypoints for the Path Feature box's **Draw Segments Path** tool (see [Draw Segments Path](#draw-segments-path)) to place onto the map later; and **Manage Segments**, a popup listing every segment type already in `settings.json` — pick one to see its segments, each with Move Up / Move Down / Delete buttons that write to `settings.json` immediately.

Pressing **Add Path Segment Type** opens a file browser (defaulting to the last directory used), then a confirm dialog showing the chosen file's folder (= type name, assumed unique) and filename, plus a note that segments are assumed modeled at 10 mm man-height. Confirming imports the STL, frames the viewport top-down on it, and expands the Settings box into **Define Corners** / **Draw Path** / **Finish Add Segment** / **Cancel**.

**Define Corners** builds the segment's own footprint polygon by hand: pressing **Add Corner** arms a one-click placement (press it again for each subsequent corner — it's not a continuous drag-drawing session), snapping to the imported mesh's convex-hull footprint edges the same way Draw Path does, plus a **sharp-corner hint** — hull vertices whose turn exceeds the Sharp Corner Threshold (default 50°, adjustable) are offered as an extra, distinctly-coloured snap target, so a rounded/filleted STL edge's many near-straight hull vertices don't get in the way of clicking its real corners. A newly placed corner's Z defaults to the whole mesh's lowest vertex. At least 3 corners are required before Finish is enabled; once 3+ exist, they become the *authoritative* footprint — Draw Path's edge snapping and its "must stay inside the footprint" check both switch from the auto convex hull to this polygon, and new snap points appear at the midpoint of every corner-to-corner edge, making it easy to place a waypoint exactly halfway between two real corners. A corner can be re-snapped onto the hull boundary afterward with its own **Snap to Edge** button, same X/Y/Z + per-axis lock editing as a waypoint (below).

**Draw Path** places waypoints that snap to the segment's footprint edges (the Corners polygon once authoritative, otherwise the convex hull) — mirroring how Path Feature lines snap to a hex's own edges. **Finish Add Segment** requires both at least 3 defined corners and at least one edge-snapped waypoint (warns "This is an end segment." if exactly one) before writing the type + segment + corners + waypoints into `settings.json`. **Cancel** discards everything in-scene without writing anything (nothing is written to `settings.json` until Finish succeeds).

Once a path has been drawn, a **Waypoints** list appears showing each point's X/Y/Z and which footprint edge (if any) it's snapped to; selecting one exposes editable X/Y/Z fields, a lock toggle per axis, and a **Snap to Edge** button that moves the point onto the nearest point of the auto convex hull's boundary in the X/Y plane — a locked axis is held fixed (the other axis is solved so the point still lands exactly on an edge), and locking both X and Y makes Snap to Edge a no-op. This precision matters because these are exactly the points **Draw Segments Path** matches up to connect two placed segments later. Redrawing the path (pressing Draw Path again) always replaces the whole waypoint list from scratch, discarding any manual edits made here — the same applies to editing corners after a path already exists, since a waypoint's recorded edge index can become stale relative to a since-changed corner. Both the drawn path and the corners polygon stay visible in the viewport once their respective modal tools exit — a connecting line plus small numbered markers ("P1", "P2", ... for waypoints; "C1", "C2", ... for corners, a distinct glyph/colour from the waypoint flags) — so you can always see what you've committed. Selecting a different row in either list highlights that point's marker (bigger, distinct colour) so you can immediately see where it sits in 3D. See **[docs/settings.md](docs/settings.md)** for the `settings.json` schema and type/segment documentation.
...
│  │              / Local Subdivision (auto-filled to 3) / Preserve Edge
...
│   ├─ path_features.py        # modal draw-feature (waypoint line) operator + texture-carve pipeline + feature list UI + link-connected-paths operator
...
│   ├─ map.py                  # pure-Python grid math + SHARED_CORNERS/EDGE_DIRECTIONS tables + edge snap points + connected-component BFS
...
17. **Link Connected Paths check** — draw a line on one tile ending at a hex-edge snap point, then draw a second line on the neighbouring tile starting from the same physical edge point (snapped) — the two segments should visually meet. Change the first line's Type/Width/Depth, select it, and press **Link Connected Paths**: the second segment's fields update to match and the seam stays continuous, while its own Name is untouched. Draw a third line that forks off an interior waypoint of the first (same-tile join) and confirm it also picks up the settings. Arrange three lines so they mutually connect in a loop across three tiles (a cycle) and confirm the button completes without hanging and updates all three exactly once. Pressing the button on a path with no shared waypoints anywhere should report "no connected paths found" and change nothing. A headless smoke test of the operator (cross-tile seam, same-tile fork, transitive chain, and the isolated-feature no-op case) lives in `tests/_headless_link_paths_check.py`: `blender --background --python tests/_headless_link_paths_check.py`.
