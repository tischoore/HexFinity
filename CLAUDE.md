...
| `flora.py` | bpy shell — modal `HEXFINITY_OT_flora_marker`: raycasts the mouse onto the map and draws a live yellow circle+dot marker (`POST_PIXEL`); Esc/RMB to exit. `is_active()` (backed by a module-level flag set/cleared in `invoke`/`_finish`) lets `panel.py` swap the button for a live "Flora active" indicator while the modal owns input — a real Close button can't be reached there since a running modal operator swallows clicks before they reach panel buttons. No placement logic yet — foundation for a future flora scatter feature |
